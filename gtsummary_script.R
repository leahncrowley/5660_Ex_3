# gtsummary assignment - Advanced Data Analytics 
# Leah N. Crowley - Fall 2023 

# Make three summary tables (t-test, ANOVA, and ANCOVA) using gtsummary and make 
# three accompanying ggplots as graphical representations of the data. Use boxplots, 
# bar charts with error bars, and regression plots as appropriate. Push project
# output to Github as a webpage.
...
# Bring in data to use for assignment: 
  ...
  anova_texas <- texas %>% filter(site == "TAMU16" | site == "TAMU8" | site == "TAMU27" | site == "TAMU74")
...
# t-test: compare the means of different variables from data recorded in two culverts: TAMU16 and TAMU8.
  table_texas1 <- texas %>%
    ...
    statistic = list(all_continuous() ~ "{mean} ({sd})", 
      ...
      modify_caption("Table 1. TX: Culvert Temperatures and Water Vapor Deficits Over Winter") %>%
      modify_header(
        update = list(
          label ~ '',
          stat_1 ~ '**TAMU16**', 
...
# Save t-test table:
  table_texas1 %>% 
    as_gt() %>%   
    gt::gtsave(filename = "tx_ttest_gts.pdf") 
...
# Save ANOVA table:
  table_texas2 %>% 
    as_gt() %>%   
    gt::gtsave(filename = "tx_anova_gts.pdf")
...
# ANCOVA measurements adjusted for WNS status of hibernaculum:
...
# Save ANCOVA table:
  table_texas3 %>% 
    as_gt() %>%   
    gt::gtsave(filename = "tx_ancova_gts.pdf")  
